...
### 2025-02-18
- 新增 Agent 流式进度事件系统（`src/agent/stream_events.py`）
- 所有 agent 节点添加 `emit_progress` 调用，支持业务级进度反馈
- API 流式输出改造：从 `astream_events` 改为 `astream(stream_mode=["custom", "updates"])`
- 修复 `plan_service.py` 图二次执行 bug（删除 `ainvoke` 二次调用，改用 `final_output` 累积状态）
...
API 层通过 `astream(stream_mode=["custom", "updates"])` 消费 Agent 模块的结构化进度事件。

**核心要点**:
- Agent 节点通过 `emit_progress()` 发送 `ProgressEvent`（定义在 `src/agent/stream_events.py`）
- `stream.py` 使用 `stream_mode=["custom", "updates"]` 替代 `astream_events(version="v2")`
- `plan_service.py` 通过可变 `final_output` dict 在流式过程中收集最终状态，**不再二次调用 `ainvoke`**
- 子图中的 custom 事件会自动冒泡到顶层 astream 消费者

**⚠️ 重要注意事项**:
- 在流式模式（`astream`）执行完成后，**绝对不要再调用 `ainvoke`**，否则图会被执行两次
- `emit_progress` 使用 try-except 静默处理，确保 `ainvoke`（非流式）模式下不会崩溃
- 前端收到的 SSE 事件中，`progress` 字段（0-100）可直接用于展示进度条
...
### 7. 流式进度与图执行
- ⚠️ **绝对不要在 `astream` 完成后再调用 `ainvoke`**——这会导致图被执行两次
- ✅ 流式模式通过 `final_output` 可变 dict 在迭代过程中累积最终状态
- ✅ 在 agent 节点中使用 `emit_progress()` 发送进度事件时，必须用 try-except 包裹（已由 `emit_progress` 内部处理）
- ✅ 子图中的 `get_stream_writer()` custom 事件会自动冒泡到顶层 `astream` 消费者
- 💡 `emit_progress` 在 `ainvoke` 调用路径下静默跳过，无需条件判断
