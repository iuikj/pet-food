...
class NoteState(AgentState):
    ...
    note_write_count: NotRequired[int]
...
def create_query_note_tool(
    ...
    @tool(
        ...
        description=description or "Query a note by name.**you must use 'ls' to know the current notes before you call this tool,**",
    )
    def query_note(
            file_name: Annotated[str, "the name of the note"],
            state: Annotated[NoteState, InjectedState],
...
class NoteMiddleware(AgentMiddleware[NoteState]):

    state_schema = NoteState
    tools = [ create_ls_tool(), create_query_note_tool(), create_update_note_tool()]

    def after_model(self, state: NoteState, runtime: Runtime[ContextT]) -> dict[str, Any] | None:
        pass
